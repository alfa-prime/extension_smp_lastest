...
async def fetch_started_data(patient: ExtensionStartedData, gateway_service: GatewayService):
    search_date_range = (patient.dis_date_range or
                         f"{settings.SEARCH_PERIOD_START_DATE} - {datetime.now().strftime('%d.%m.%Y')}")
